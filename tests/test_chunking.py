...
from app.services.config import CHUNK_SIZE
...
def test_document_with_less_characters():
    ...
    fake_doc1= Document(page_content="This is a short document with real words. " * 5)
    chunks= splitting_docs([fake_doc1])
    assert len(chunks)==1
...
def test_multiple_docs():
    ...
    fake_doc1= Document(page_content="*"*400)
    fake_doc2= Document(page_content=";"*400)
...
def test_content_lost():
    ...
    fake_doc= Document(page_content="Hi, myself Puneeth. I am an Applied Computer Science graduate student.")
    chunks= splitting_docs([fake_doc])
    all_content = "".join(c.page_content for c in chunks)
